...
def main(argv):
    ...
    raw_biomarkers_parquet_path = argv[2]
...
def transform_gene_helper(
    ...
    biomarkers_genes_df = raw_biomarkers_df.select("biomarker").drop_duplicates()
    biomarkers_genes_df = biomarkers_genes_df.withColumnRenamed("biomarker", "symbol")
    # biomarkers data does not have ensembl_gene_id or ncbi_gene_id so adding black values here to help have
    # consistency
    biomarkers_genes_df = biomarkers_genes_df.withColumn("ensembl_gene_id", lit(""))
    biomarkers_genes_df = biomarkers_genes_df.withColumn("ncbi_gene_id", lit(""))
    ...
    genes_df = cna_genes_df.union(biomarkers_genes_df).union(expression_genes_df).union(mutation_genes_df)
