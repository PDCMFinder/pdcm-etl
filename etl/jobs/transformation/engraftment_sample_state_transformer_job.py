...
def main(argv):
    ...
    engraftment_sample_state_df = transform_engraftment_sample_state(raw_model_df)
    engraftment_sample_state_df.write.mode("overwrite").parquet(output_path)


def transform_engraftment_sample_state(raw_model_df: DataFrame) -> DataFrame:
...
def get_engraftment_sample_state_from_model(raw_model_df: DataFrame) -> DataFrame:
    return raw_model_df.select(init_cap_and_trim_all("sample_state").alias("name"))
