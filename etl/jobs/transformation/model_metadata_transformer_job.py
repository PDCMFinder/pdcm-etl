...
def main(argv):
    ...
    :param list argv: the list elements should be:
                    ...
                    [8]: Parquet file path with the provider_group transformed data.
                    [9]: Output file
    """
    ...
    provider_group_parquet_path = argv[8]
    output_path = argv[9]

    spark = SparkSession.builder.getOrCreate()
    model_df = spark.read.parquet(model_parquet_path)
    search_index_patient_sample_df = spark.read.parquet(
        search_index_patient_sample_parquet_path
    )
    xenograft_model_specimen_df = spark.read.parquet(
        xenograft_model_specimen_parquet_path
    )
    ...
    treatment_aggregator_helper_df = spark.read.parquet(
        treatment_aggregator_helper_parquet_path
    )
...
def transform_model_metadata(
    ...
    model_df = model_df.join(
        search_index_patient_sample_df,
        on=["pdcm_model_id", Constants.DATA_SOURCE_COLUMN],
    )
    ...
    # to search_index
    treatment_aggregator_helper_df = treatment_aggregator_helper_df.withColumnRenamed(
        "model_id", "pdcm_model_id"
    )
    model_df = model_df.join(
        treatment_aggregator_helper_df, on=["pdcm_model_id"], how="left"
    )
    ...
    # Add information about the provider (`model_generator` and `view_data_at`)
    provider_group_df = provider_group_df.select(
        "abbreviation", "model_generator", "view_data_at"
    )
    model_df = model_df.join(
        provider_group_df,
        on=[model_df.data_source == provider_group_df.abbreviation],
        how="left",
    )
...
def get_formatted_model(model_df: DataFrame) -> DataFrame:
    # Renaming columns
...
def add_quality_assurance_data(
    df: DataFrame, quality_assurance_df: DataFrame
) -> DataFrame:
    quality_assurance_df = quality_assurance_df.withColumn(
        "json_entry",
        concat(
            lit("{"),
            lit('"validation_technique": '),
            lit('"'),
            col("validation_technique"),
            lit('", '),
            lit('"description": '),
            lit('"'),
            col("description"),
            ...
            lit('"'),
            col("validation_host_strain_nomenclature"),
            lit('", '),
            lit('"morphological_features": '),
            lit('"'),
            col("morphological_features"),
            lit('", '),
            lit('"SNP_analysis": '),
            lit('"'),
            col("SNP_analysis"),
    ...
    quality_data_per_model_df = quality_assurance_df.groupby("model_id").agg(
        concat_ws(", ", collect_list(quality_assurance_df.json_entry)).alias(
            "quality_assurance"
        )
    )
    ...
    df = df.join(
        quality_data_per_model_df,
        df.pdcm_model_id == quality_data_per_model_df.model_id,
        how="left",
    )
    df = df.drop("model_id")
    return df
...
def add_xenograft_model_specimen_data(
    df: DataFrame, xenograft_model_specimen_df: DataFrame
) -> DataFrame:
    ...
    xenograft_model_specimen_df = xenograft_model_specimen_df.withColumn(
        ...
        concat(
            lit("{"),
            lit('"host_strain_name": '),
            lit('"'),
            col("host_strain_name"),
            lit('", '),
            lit('"host_strain_nomenclature": '),
            lit('"'),
            col("host_strain_nomenclature"),
            lit('", '),
            lit('"engraftment_site": '),
            lit('"'),
            col("engraftment_site"),
            lit('", '),
            lit('"engraftment_type": '),
            lit('"'),
            col("engraftment_type"),
            lit('", '),
            lit('"engraftment_sample_type": '),
            lit('"'),
            col("sample_type"),
            lit('", '),
            lit('"engraftment_sample_state": '),
            lit('"'),
            col("sample_state"),
            lit('", '),
            lit('"passage_number": '),
    ...
    xenograft_model_specimen_per_model_df = xenograft_model_specimen_df.groupby(
        "model_id"
    ).agg(
        concat_ws(", ", collect_list(xenograft_model_specimen_df.json_entry)).alias(
            "xenograft_model_specimens"
        )
    )
    xenograft_model_specimen_per_model_df = (
        xenograft_model_specimen_per_model_df.withColumn(
            "xenograft_model_specimens",
...
def add_images_data(df: DataFrame, model_image_df: DataFrame) -> DataFrame:
    ...
    images_per_model_df = images_per_model_df.withColumn(
        "model_images", concat(lit("["), col("model_images"), concat(lit("]")))
    )
    df = df.join(
        images_per_model_df,
        df.pdcm_model_id == images_per_model_df.model_id,
        how="left",
    )
    df = df.drop("model_id")
    return df
...
def add_dataset_available(
    df: DataFrame, search_index_molecular_char_df: DataFrame
) -> DataFrame:
    ...
    search_index_molecular_char_df = search_index_molecular_char_df.withColumn(
        ...
        when((col("molecular_characterisation_type") == "biomarker"), "bio markers")
        .when(
    ...
    model_mol_char_availability_df = search_index_molecular_char_df.groupby(
        "model_id"
    ).agg(collect_set("molecular_characterisation_type").alias("dataset_available"))

    df = df.join(
        model_mol_char_availability_df,
        on=[df.pdcm_model_id == model_mol_char_availability_df.model_id],
        how="left",
    )
    ...
    df = df.withColumn(
        ...
        when(
            ...
            when(
                col("dataset_available").isNotNull(),
                concat(col("dataset_available"), array(lit("model treatment"))),
            ).otherwise(array(lit("model treatment"))),
    ...
    df = df.withColumn(
        ...
        when(
            ...
            when(
                col("dataset_available").isNotNull(),
                concat(col("dataset_available"), array(lit("patient treatment"))),
            ).otherwise(array(lit("patient treatment"))),
        ).otherwise(col("dataset_available")),
    )
    ...
    df = df.withColumn(
        ...
        when(
            ...
            when(
                col("dataset_available").isNotNull(),
                concat(col("dataset_available"), array(lit("publication"))),
            ).otherwise(array(lit("publication"))),
        ).otherwise(col("dataset_available")),
    )
...
def add_custom_treatment_type_column(model_df: DataFrame) -> DataFrame:
    ...
    model_df = model_df.withColumn(
        ...
        lower_and_trim_all("treatment_naive_at_collection"),
    )
    ...
    model_df = model_df.withColumn(
        ...
        when(
            col("treatment_naive_at_collection").isin(
                "not provided", "not collected", "no"
            ),
            None,
        )
        .when(col("treatment_naive_at_collection").isin("yes"), lit("Treatment naive"))
        .otherwise(col("treatment_naive_at_collection")),
    )

    # Make patient_treatment_status an array
    model_df = model_df.withColumn(
        "tmp_patient_treatment_status", split(col("tmp_patient_treatment_status"), ",")
    )
