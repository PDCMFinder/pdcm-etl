...
def add_treatment_links(treatment_df: DataFrame, resources_df: DataFrame):
    ...
    treatment_df = treatment_df.join(treatment_names_links_column_df, on=["name"], how="left")
...
def find_chembl_links(treatment_names_df: DataFrame, resource) -> DataFrame:
    ...
    treatment_names_df = treatment_names_df.withColumn("chembl_id", get_chembl_id_udf("name"))
