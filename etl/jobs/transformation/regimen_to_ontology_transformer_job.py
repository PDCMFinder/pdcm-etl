...
def main(argv):
    ...
    :param list argv: the list elements should be:
                    ...
                    [2]: Parquet file path with ontology term regimen data
    ...
    treatment_to_ontology_df = transform_regimen_to_ontology(
        treatment_df, ontology_term_regimen_df, treatment_mappings_df)
    treatment_to_ontology_df.write.mode("overwrite").parquet(output_path)


def transform_regimen_to_ontology(
        treatment_df: DataFrame, ontology_term_regimen_df: DataFrame, treatment_mappings_df: DataFrame) -> DataFrame:
    treatment_df = lower_treatment_columns(treatment_df)
    treatment_mappings_df = lower_mapping_column_values(treatment_mappings_df)

    treatment_to_ontology_df = link_regimen_to_ontology(
        treatment_df, ontology_term_regimen_df, treatment_mappings_df)
...
def link_regimen_to_ontology(
        treatment_df: DataFrame, ontology_term_regimen_df: DataFrame, treatment_mappings_df: DataFrame) -> DataFrame:

    ontology_term_term_regimen_df = ontology_term_regimen_df.withColumnRenamed("id", "ontology_term_id")
    ...
    treatment_df = treatment_df.withColumnRenamed("id", "regimen_id")
    ...
    treatment_mappings_df = treatment_mappings_df.join(ontology_term_term_regimen_df, on=['term_url'], how='left')
    # We are only interested in the records where there was a match in the ontology
    treatment_mappings_df = treatment_mappings_df.where("term_id is not null")
    ...
    # Most records in treatment_df are individual treatments and not regimens so we need to filter just the
    # cases where there was a match
    treatment_to_ontology_df = treatment_to_ontology_df.where("term_id is not null")

    return treatment_to_ontology_df.select("regimen_id", "ontology_term_id")
