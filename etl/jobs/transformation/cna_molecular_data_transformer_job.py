...
from etl.jobs.util.external_resource_links_builder import add_links_column
...
def main(argv):
    ...
    cna_molecular_data_df = transform_cna_molecular_data(
        molecular_characterization_df, raw_cna_df, raw_external_resources_df, gene_markers_parquet_path)
...
def transform_cna_molecular_data(
    ...
    cna_df = add_external_resources_links_column(cna_df, raw_external_resources_df)
    return cna_df
...
def add_external_resources_links_column(cna_df: DataFrame, raw_external_resources_df: DataFrame):
    column_to_link = {"name": "hgnc_symbol", "source_columns": ["hgnc_symbol"], "type": "Gene"}

    cna_df = add_links_column(cna_df, [column_to_link], raw_external_resources_df)
    return cna_df
