...
def transform_treatment_to_ontology(
    ...
    treatment_mappings_df: DataFrame,
) -> DataFrame:
...
def lower_treatment_columns(treatment_df: DataFrame) -> DataFrame:
    treatment_df = treatment_df.withColumn(
        "datasource", lower_and_trim_all("data_source")
    )
...
def lower_mapping_column_values(treatment_mappings_df: DataFrame) -> DataFrame:
    treatment_mappings_df = treatment_mappings_df.withColumn(
        "datasource", lower_and_trim_all("datasource")
    )
    treatment_mappings_df = treatment_mappings_df.withColumn(
...
def link_treatments_to_ontology(
    ...
    treatment_mappings_df = treatment_mappings_df.withColumnRenamed(
        "id", "ontology_term_id"
    )
    treatment_mappings_df = treatment_mappings_df.withColumnRenamed(
        "mapped_term_url", "term_url"
    )
    ...
    treatment_mappings_df = treatment_mappings_df.join(
        ontology_term_treatment_df, on=["term_url"], how="left"
    )

    # The second join uses the actual treatment data (so this changes according to the providers we are processing)
    treatment_to_ontology_df = treatment_df.join(
        treatment_mappings_df, on=["datasource", "treatment"], how="left"
    )
