...
submit-dev: build
	source venv/bin/activate && LUIGI_CONFIG_PATH='luigi-cluster-dev.cfg'  PYTHONPATH='.' TMPDIR='/nfs/production/tudor/pdcm/tmp' PYSPARK_PYTHON="/hps/software/users/tudor/pdcm/pdcm-etl/venv/bin/python3" time luigi --module etl.workflow.main --scheduler-host ves-ebi-d9.ebi.ac.uk PdcmEtl --workers 10

submit-prod: build
	source venv/bin/activate && LUIGI_CONFIG_PATH='luigi-cluster-prod.cfg'  PYTHONPATH='.' TMPDIR='/nfs/production/tudor/pdcm/tmp' PYSPARK_PYTHON="/hps/software/users/tudor/pdcm/pdcm-etl/venv/bin/python3" time luigi --module etl.workflow.main --scheduler-host ves-ebi-d9.ebi.ac.uk PdcmEtl --workers 10

rebundle-submit-dev: clean libs build        ##@submit Submit luigi workflows to the cluster
	source venv/bin/activate && LUIGI_CONFIG_PATH='luigi-cluster-dev.cfg'  PYTHONPATH='.' TMPDIR='/nfs/production/tudor/pdcm/tmp'  PYSPARK_PYTHON="/hps/software/users/tudor/pdcm/pdcm-etl/venv/bin/python3" time python etl/workflow/main.py --scheduler-host ves-ebi-d9.ebi.ac.uk PdcmEtl --workers 10

rebundle-submit-prod: clean libs build        ##@submit Submit luigi workflows to the cluster
	source venv/bin/activate && LUIGI_CONFIG_PATH='luigi-cluster-prod.cfg'  PYTHONPATH='.' TMPDIR='/nfs/production/tudor/pdcm/tmp'  PYSPARK_PYTHON="/hps/software/users/tudor/pdcm/pdcm-etl/venv/bin/python3" time python etl/workflow/main.py --scheduler-host ves-ebi-d9.ebi.ac.uk PdcmEtl --workers 10
